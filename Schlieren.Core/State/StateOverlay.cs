...
public sealed class StateOverlay : IGlobalState
{
    ...
    /// <summary>
    /// Restore state to a previously taken snapshot. Clears the overlay buffer
    /// and restores the parent state. Used for CREATE frame rollback on EIP-3541/EIP-170.
    /// </summary>
    public void RestoreSnapshot(IDictionary<Address, Account> snapshot)
    {
        // Clear this overlay's pending writes
        _buffer.Clear();
