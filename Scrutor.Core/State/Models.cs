...
public sealed class Transaction : IComparable<Transaction>
{
        ...
        int sIndex)
    {
        ...
        var unsignedRlp = Scrutor.Core.Encoding.RlpEncoder.EncodeList(unsignedItems);
    ...
    private static byte[] ComputeLegacySigningHash(List<Scrutor.Core.Encoding.RlpItem> items, int v)
    {
        ...
        if (v >= 35)
        {
            ...
            var chainId = (v - 35) / 2;
            var unsigned = new List<Scrutor.Core.Encoding.RlpItem>(9);
            for (int i = 0; i < 6; i++) unsigned.Add(items[i]);
            unsigned.Add(EncodeUintRlp((ulong)chainId));
            unsigned.Add(new Scrutor.Core.Encoding.RlpItem { Data = ReadOnlyMemory<byte>.Empty });
            unsigned.Add(new Scrutor.Core.Encoding.RlpItem { Data = ReadOnlyMemory<byte>.Empty });
            return CryptoUtils.Keccak256(Scrutor.Core.Encoding.RlpEncoder.EncodeList(unsigned));
        }

        // Homestead: sign over first 6 fields only
        var six = items.GetRange(0, 6);
        return CryptoUtils.Keccak256(Scrutor.Core.Encoding.RlpEncoder.EncodeList(six));
    }

    private static Scrutor.Core.Encoding.RlpItem EncodeUintRlp(ulong value)
    {
        if (value == 0)
            return new Scrutor.Core.Encoding.RlpItem { Data = ReadOnlyMemory<byte>.Empty };
        ...
        return new Scrutor.Core.Encoding.RlpItem { Data = buf.Slice(start).ToArray() };
    }
