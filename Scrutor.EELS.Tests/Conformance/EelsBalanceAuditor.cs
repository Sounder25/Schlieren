...
/// flags exactly which term produces the wrong result in Scrutor vs the fixture.
///
...
public static class EelsBalanceAuditor
{
        ...
        string mismatch)
    {
        ...
        // Scrutor charges: gasLimit × effectiveGasPrice (not maxFeePerGas — type-2 refunds difference later)
        ...
        // Expected contribution = term4_exact_refund; if Scrutor got it wrong the
        ...
        if (term4_exact_refund > 0)
        {
            ...
            {
                // Adding T4 made it worse — Scrutor is double-counting the refund
                faultedTerms.Add(
            ...
            {
                // Delta exactly equals the expected refund — Scrutor skipped it entirely
                faultedTerms.Add(
    ...
    public static string RenderMarkdown(BalanceAuditReport r)
    {
        ...
        foreach (var row in r.LedgerRows.Take(5))
        {
            ...
            sb.AppendLine($"  scrutor actual                = {row.ActualBalance}");
