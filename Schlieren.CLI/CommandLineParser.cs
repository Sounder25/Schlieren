...
{
    /// <summary>
    /// Builds the 'schlieren node' subcommand with all Anvil-compatible options.
    /// </summary>
    public static Command BuildNodeCommand()
    {
        ...
        rootCommand.Description = "Schlieren node — Anvil-compatible local Ethereum node";
    ...
    public static Task<(NodeConfiguration? Config, int ExitCode)> ParseNodeContextAsync(InvocationContext context)
    {
        ...
        {
            // Auto-detect schlieren.config.json in the current working directory (workspace config).
            var workspaceConfig = Path.Combine(Directory.GetCurrentDirectory(), "schlieren.config.json");
