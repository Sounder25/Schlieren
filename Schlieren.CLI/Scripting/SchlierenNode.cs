...
namespace Schlieren.CLI.Scripting;
...
{
    private readonly SchlierenNode _node;
    private readonly string      _address;
    private readonly JsonElement _abi;

    internal Contract(SchlierenNode node, string address, JsonElement abi)
    {
        _node    = node;
        _address = address;
        _abi     = abi;
...
// ─────────────────────────────────────────────────────────────────────────────
//  SchlierenNode — the primary script host context object
// ─────────────────────────────────────────────────────────────────────────────

/// <summary>
/// Injected into every .csx script as `node`.
/// Wraps JSON-RPC with Schlieren-specific conveniences: gas records, snapshots,
/// fork inspection, and trace hooks.
/// </summary>
public sealed class SchlierenNode : IAsyncDisposable
{
    ...
    public SchlierenNode(string rpcUrl, string artifactsDir)
    {
        _rpcUrl       = rpcUrl;
    ...
    private async Task<TxResult> SendRaw(string from, string? to, string data, BigInteger? value)
    {
        ...
        // Poll for receipt (automine is instant on Schlieren dev-node)
    ...
    private JsonElement LoadArtifact(string contractName)
    {
        ...
        if (!File.Exists(path))
            throw new FileNotFoundException(
                $"Artifact not found: {path}. Run 'schlieren compile' first.");
    ...
    private static byte[] Keccak256Ascii(string s)
    {
        ...
        // Use Schlieren.Core's keccak if reachable — otherwise inline SHA3 via
        ...
        // Real Keccak available via Schlieren.Core.Primitives.CryptoUtils.Keccak256.
        return Schlieren.Core.Primitives.CryptoUtils.Keccak256(bytes);
    }
