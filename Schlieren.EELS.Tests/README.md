# Schlieren EELS Harness

This project runs published EELS `state_test` fixtures against Schlieren.
...
Failed cases in `EelsTaxonomyDrill` are fed through `DivergenceDiagnostics` (Schlieren.Core)
...
```powershell
dotnet test Schlieren.EELS.Tests --filter "EelsTaxonomyDrill"
...
```powershell
dotnet test .\Schlieren.EELS.Tests\Schlieren.EELS.Tests.csproj --nologo
```
